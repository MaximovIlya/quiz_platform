...
export default function QuizHistoryPage() {
  ...
  useEffect(() => {
    if (!params.id) return;
    apiFetch(`/quiz/${params.id}/history`)
      .then((r) => r.json())
  ...
  const grad = CATEGORY_GRADIENTS[data.category] ?? "linear-gradient(165deg,#0077FF,#005CC4)";
      ...
      <div className="page-body" style={{ maxWidth: 880, margin: "0 auto", padding: "40px 32px" }}>
        ...
        <div style={{ display: "flex", gap: 18, alignItems: "center", marginBottom: 32 }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.02em", margin: "0 0 6px" }}>{data.quizTitle}</h1>
            <div style={{ fontSize: 14, color: "#909499" }}>
              {data.games.length} {plural(data.games.length, ["игра", "игры", "игр"])} · {totalPlayers} {plural(totalPlayers, ["участник", "участника", "участников"])} всего
            </div>
          </div>
        </div>
        ...
        {data.games.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {data.games.map((g) => (
              <Link key={g.sessionId} href={`/results/${g.sessionId}`} style={{
